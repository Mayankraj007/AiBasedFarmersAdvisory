...
// 🌱 SOIL HEALTH ADVISORY ROUTE (NEW)
app.post("/api/soil-advisory", async (req, res) => {
    try {
        ...
        // Fetch weather forecast (REUSING SAME API)
        const response = await axios.get(BASE_URL, {
            params: { key: WEATHER_API_KEY, q: location, days: 5 }
        });
        ...
        const advisory = await generateSoilAdvisory(soilType, soilPH, nitrogen, phosphorus, potassium, moisture, forecast);

        res.json({ location: response.data.location.name, soilType, forecast, advisory });
